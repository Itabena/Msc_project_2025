...
def pressecion_plots_2(rp1,rp2,rp_wegg1,rp_wegg2,Gr_L_list,Pn_L_list,Pw_L_list,Pwegg_L_list,Gr_parb_prec_list,Pn_parb_prec_list,Pw_parb_prec_list,Pwegg_parb_prec_list,g1,g2,f1,f2,type=1):
    ...
    if type==2:
        ...
        plt.plot(Pw_L_list[:g2]-Pw_L_list[0], np.array(Pw_parb_prec_list[:g2]), 'g-.', label='Pw')
        plt.plot(Pwegg_L_list[:g2]-Pwegg_L_list[0], np.array(Pwegg_parb_prec_list[:g2]), 'r--', label='Pwegg')
        plt.xscale('log')
        plt.ylim(0,16)
        plt.xlim(10**(-5),10**(-1))
        plt.legend()
...
# new_main(1, 2, [0,4,5,6,7,8,9], rangelist=rangelist)
# #             except Exception as e:
...
##good for precession perpose [0,4,5,6,7,8],[0,4,5,6,7,8,9]
# conds_list=[[0,1,4,5,6,7,12,13],[0,1,4,5,6,7,12,13],[0,1,4,5,6,7,12,13],[0,1,4,5,6,7,12,13],[0,1,4,5,6,7,12,13]]
# rs=4*np.sqrt(6)-9
# rss=[rs,2,1,0.2,1.5]
# rsss=2*np.ones(len(rss))
# N1_list=[int(1) for i in range(len(conds_list))]
# N11_list=[i for i in range(len(conds_list))]

# conds_list=[[0,4,5,6,7,8],[0,4,5,6,7,8,9]]
# conds_list_2=[[0,4,5,6,7,8],[0,4,5,6,7,8,9]]
# # rs=4*np.sqrt(6)-9
# rss1=[2,2]
# rsss=2*np.ones(len(rss1))
# N1_list1=[int(5) for i in range(len(conds_list))]

conds_list=[[0,3,4,5,6,7,8],[0,3,4,5,6,7,8],[0,3,4,5,6,7,8],[0,3,4,5,6,7,8],[0,3,4,5,6,7,8],[0,3,4,5,6,7,8],[0,3,4,5,6,7,8]] # no rss scan only N1 rs=2

conds_list2=[[0,1,4,5,6,7,8],[0,1,4,5,6,7,8],[0,1,4,5,6,7,8]]# rss scan and N1 scan

conds_list3=[[0,1,4,5,6,7,8,12],[0,1,4,5,6,7,8,12],[0,1,4,5,6,7,8,12],[0,1,4,5,6,7,8,12],[0,1,4,5,6,7,8,12],[0,1,4,5,6,7,8,12],[0,1,4,5,6,7,8,12]]# no rss scan only N1 rs=2


rs=4*np.sqrt(6)-9
rss1=[2,2,2]
rss2=2*np.ones(len(conds_list3))
N1_list2=[0,1,3,4,5,6,7]
N1_list1=[5,6,7]

# N11_list=[i for i in range(len(conds_list))]

#condition 3 does not help also condition 9
...
u_case_1 = [u(r, 7, coefficients_case_1, 2) for r in r_values]
u_case_2 = [u(r, 1, coefficients_case_2, 2) for r in r_values]
u_pw = [u_pw(r) for r in r_values]
u_wegg = [u_wegg(r) for r in r_values]
u_newtonian = [-1 / r for r in r_values]

# Plot
plt.figure(figsize=(10, 6))
plt.plot(r_values, u_case_1, label="PN Case 1 (N1=7)", linestyle='-', color='blue')
plt.plot(r_values, u_case_2, label="PN Case 2 (N1=1)", linestyle='--', color='orange')
plt.plot(r_values, u_pw, label="Pw", linestyle='-.', color='green')
plt.plot(r_values, u_wegg, label="Wegg", linestyle=':', color='red')
plt.plot(r_values, u_newtonian, label="-1/r (Newtonian)", linestyle='-', color='black')
...
plt.xlim(2, 20)
plt.ylim(-1.5, 0.1)
plt.title("Comparison of Potentials", fontsize=20)
plt.legend(fontsize=10)
plt.grid()
# Save the plot as a PNG with a transparent background
plt.savefig("C:/Users/itama/Desktop/comparison_of_potentials.png", transparent=True)
plt.show()
...
####This run for N1=7,rs=2,conds=[0, 1, 4, 5, 6, 7, 8, 12] The coefficients are [ 1017.25042927 -1959.14575675  2070.37282323 -1296.67371847 481.76134414   -98.39906795     8.52931077  -222.69536425]####
####This run for N1=1,rs=2,conds=[0, 1, 4, 5, 6, 7, 8, 12] The coefficients are [ 2.50098177e+01 -2.40098177e+01 -4.70196353e+01 -2.41486676e+02 2.03915849e+03 -1.44762076e+04  4.15915624e+04 -5.36285891e+04]####
...
#     zero_points_gr = np.where(np.isclose(gr_potential, 0, atol=1e-4))
#     zero_points_pn = np.where(np.isclose(pn_potential, 0, atol=1e-4))
#     zero_points_wegg = np.where(np.isclose(wegg_potential, 0, atol=1e-4))

#     fig, ax = plt.subplots(figsize=(10, 7))

#     # GR angular velocity contours
#     c1 = ax.contour(R, L_grid, gr_angular_velocity, levels=20, cmap='coolwarm',locator=ticker.LogLocator(), linewidths=1.5)
#     plt.colorbar(c1, ax=ax, label='GR Angular Velocity')

#     # PN angular velocity contours
#     c2 = ax.contour(R, L_grid, pn_angular_velocity, levels=20, cmap='cool',locator=ticker.LogLocator(), linewidths=1.5, linestyles='--')
#     plt.colorbar(c2, ax=ax, label='PN Angular Velocity')

#     # Wegg angular velocity contours
#     c3 = ax.contour(R, L_grid, wegg_angular_velocity, levels=20, cmap='autumn', locator=ticker.LogLocator(), linewidths=1.5, linestyles='-.')
#     plt.colorbar(c3, ax=ax, label='Wegg Angular Velocity')

#     ax.legend(['GR Zero Potential (Dashed)', 'PN Zero Potential (Solid)', 'Wegg Zero Potential (Dot-Dashed)'], loc='upper right')
#     # Zero potential lines
#     ax.plot(R[zero_points_gr], L_grid[zero_points_gr], 'k-', label='GR Zero Potential Line')
#     ax.plot(R[zero_points_pn], L_grid[zero_points_pn], 'r--', label='PN Zero Potential Line')
#     ax.plot(R[zero_points_wegg], L_grid[zero_points_wegg], 'g-.', label='Wegg Zero Potential Line')

#     ax.set_title('Combined Angular Velocity Heatmap for GR, PN, and Wegg Potentials')
#     ax.set_xlabel('r')
#     ax.set_ylabel('L')
#     ax.legend()
#     ax.grid()
...
# Debugging: Check the integrand and root-finding for large L
def debug_precession_integrand(N1, rs, conds, L_values, r_min=4, r_max=1000, num_points=10000):
    """
    For a given potential (N1, rs, conds), plot the integrand and the root equation for large L values.
    This helps diagnose why the precession integral gets noisy.
    """
    x = Solve_coeffs(N1, rs, conds)[0]
    print(f"Coefficients for N1={N1}, rs={rs}, conds={conds}: {x}")
    r_grid = np.linspace(r_min, r_max, num_points)
    for L in L_values:
        # Find periapsis numerically
        def root_eq(r):
            return - 2*u(r, N1, x, rs) - (L**2)/(r**2)
        try:
            rp_guess = (L**2)/2
            rp = fsolve(root_eq, rp_guess)[0]
        except Exception as e:
            print(f"Root finding failed for L={L}: {e}")
            continue

        # Compute the integrand
        def integrand(r):
            val =  - 2*u(r, N1, x, rs) - (L**2)/(r**2)
            return L / (r**2 * np.sqrt(np.abs(val))) if val > 0 else np.nan

        integrand_vals = np.array([integrand(r) for r in r_grid])
        root_vals = np.array([root_eq(r) for r in r_grid])
        print(f"Root periapsis for L={L}: {rp:.2f}")
        print(f"Integrand values for L={L}: {integrand_vals}")
        print(f"Root equation values for L={L}: {root_vals}")

        plt.figure(figsize=(12,5))
        plt.subplot(1,2,1)
        # plt.plot(r_grid, root_vals,'c', label='Root Eq')
        # plt.axhline(0, color='k', linestyle='--')
        # plt.axvline(rp, color='r', linestyle=':', label=f'Rp={rp:.2f}')
        # plt.title(f'Root Equation for L={L}')
        # plt.xlabel('r')
        # plt.ylabel('2E-2u(r)-L^2/r^2')
        # plt.legend()
        # plt.grid()

        plt.subplot(1,2,2)
        plt.plot(r_grid, integrand_vals,'b', label='Integrand')
        plt.title(f'Integrand for L={L}')
        plt.xlabel('r')
...
# Example usage:
# Large L values to check
L_check = [20, 40, 80, 160]
debug_precession_integrand(
    N1=1,
    rs=2,
    conds=[0,1,2,4,5,6,7,8,12],
    L_values=L_check,
    r_min=4,
    r_max=100,
    num_points=2000
